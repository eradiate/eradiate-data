...
class ExternalPathResolver():
    ...
    def reset(self):
        ...
        data_dir = os.getenv("ERADIATE_DATA_DIR")
        eradiate_dir = os.getenv("ERADIATE_DIR")

        data_path = None
        if eradiate_dir is None and data_dir is None:
            data_path = Path.cwd() / "resources" / "data"
        elif data_dir is None:
            data_path = Path(eradiate_dir) / "resources" / "data"
        else:
        ...
        if not data_path.exists():
            raise ValueError(
                "The path provided for eradiate data does not exist. Please check "
                f"if the directory {data_path} exists."
            )
